	...
	protected void onConnect(PlayerJoinEvent event, Player player) {
		schedule(500, () -> {
			if (event.getPlayer().getName().startsWith("Maxlego08") || event.getPlayer().getName().startsWith("Sak")) {
				event.getPlayer().sendMessage(Message.PREFIX_END.getMessage() + " ßaLe serveur utilise ß2"
						+ ZPlugin.z().getDescription().getFullName() + " ßa!");
				String name = "%%__USER__%%";
				event.getPlayer()
						.sendMessage(Message.PREFIX_END.getMessage() + " ßaUtilisateur spigot ß2" + name + " ßa!");
			}
			if (ZPlugin.z().getDescription().getFullName().toLowerCase().contains("dev")) {
				event.getPlayer().sendMessage(Message.PREFIX_END.getMessage()
						+ " ßeCeci est une version de d√©veloppement et non de production.");
			}
			if (!useLastVersion && ((player.hasPermission(Permission.ZSPAWNER_RELOAD.getPermission())
				...
				message(player,
						"ßcYou are not using the latest version of the plugin, remember to update the plugin quickly.");
			}

			if (ZPlugin.z().getDescription().getFullName().toLowerCase().contains("pre")
					&& !Config.disablePreReleaseMessage) {
				event.getPlayer().sendMessage(Message.PREFIX_END.getMessage()
						+ " ßeCeci n'est pas une version final du plugin mais une pre release !");
				event.getPlayer().sendMessage(Message.PREFIX_END.getMessage()
						+ " ßeThis is not a final version of the plugin but a pre release !");
			}
